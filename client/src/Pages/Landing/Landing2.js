...
function Landing2() {
    ...
    const renderBottomCards = bottomCards.map( c => {
        return <div key={c.id} className='flex border-black p-5 '>

                    <div className='w-1/2 p-1'>
                        <img className='object-fit max-h-[200px] ml-auto' alt='card' src={c.image} />
                    </div>
    ...
    const renderRightPanelImages = rightPanelImages.map( i =>{
        return <div key={i.id} className='py-5 px-10 h-1/3'>
                    <img className='image max-w-[375px] max-h-[192px] mx-auto' alt='bird' src={i.image} />
                </div>
    })
            ...
            <div id='leftPanel'
                className='w-full  h-2/3'>
                    ...
                    <div id='leftPanel top image-container' 
                        ...
                        <img className='object-cover h-full w-full max-h-[400px]' alt='bird' src='https://images.unsplash.com/photo-1601852816455-e1341014b750?auto=format&fit=crop&q=80&w=1744&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D' />
                    </div>
